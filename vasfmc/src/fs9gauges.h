...
// modified spring 2004 by Arne Bartels arne.bartels@nwn.de:
//  - definitions imported from fs2k2gauges.h
//  - some redefinitions for BCC and GCC compability reasons
...
//some definitions for backward code compability:
#define	KEY_ADF1_RADIO_FRACT_DEC_CARRY	KEY_ADF1_RADIO_TENTHS_DEC
#define	KEY_ADF1_RADIO_FRACT_INC_CARRY	KEY_ADF1_RADIO_TENTHS_INC

//
// Common Flight Simulator Data Types
//

// changed for compatibility with GCC (redefinition from <windef.h>)
#ifndef __GNUC__
	typedef	VOID			*PVOID;
#endif
...
// changed for compatibility with GCC (redefinition from <winnt.h>)
#if !defined(_WINNT_) && !defined(_WINNT_H)		// NOT PORTABLE!
typedef char    CHAR;
...
#define		ROUND( val ) ((int)floor((double)(val)+0.5))
#define		SIGNUM( val ) ( (val) > 0 ? 1 : (val) < 0 ? -1 : 0)
//a mathematical correct float modulo operation, fmod() doesn't work properly for negative values
//MODULO (
#define		MODULO(x,y)			( (double)(x) - (double)(y) * ( floor( (double)(x) / (double)(y) )  ) )
#define		BALANCED_MOD(x,y)	( (double)(x) - (double)(y) * ( floor( 0.5+(double)(x) / (double)(y) ) ) )
...
typedef struct  GAUGEHDR
{
	...
	PVOID					query_routine;               
	PVOID					install_routine;             
	PVOID					initialize_routine;          
	PVOID					update_routine;              
	PVOID					generate_routine;            
	PVOID					draw_routine;                
	PVOID					kill_routine;                
	PVOID					reserved1;
	UINT32					size_x_mm;           
...
typedef struct ELEMENT_SLIDER
{
	...
	PVOID                   source_var_x_cb;     
	FLOAT64                 scale_x;     
	PVOID                   lag_fn_x;                                             
	MODULE_VAR              source_var_y;
	PVOID                   source_var_y_cb;     
	FLOAT64                 scale_y;     
	PVOID                   lag_fn_y;    
...
#define GAUGEHDR_FOR_MOUSE_CALLBACK(PIXPOINT)	((PIXPOINT) ? ((PGAUGEHDR)(((PMOUSECALLBACK)(PIXPOINT))->user_data)) : NULL)
...
#define MOUSE_TOOLTIP_ARG_STRING_FUNCT(VAR, FUNCT) MOUSE_TOOLTIP_ARG((VAR), 1, NULL, NULL, NULL, NULL, NULL, (FUNCT))
#define MOUSE_TOOLTIP_ARG_NUMBER_MAP(VAR, MAP) MOUSE_TOOLTIP_ARG((VAR), 1, (MAP), NULL, NULL, NULL, NULL, NULL)
...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
#define	MAKE_PLIST( name , next_header )										\
		PELEMENT_HEADER		name[] = {											\
											(PELEMENT_HEADER) (next_header) ,		\
											NULL								\
										};										\



#define	PLIST_TABLE_BEGIN( name )								\
		PELEMENT_HEADER		name[] = {

#define	PLIST_TABLE_ENTRY( name )								\
		(PELEMENT_HEADER)name ,

#define	PLIST_TABLE_END( )										\
		NULL};


#define	FAILURE_LIST_BEGIN( failure_rec_name )							\
		FAILURE_RECORD	failure_rec_name[]={

#define	FAILURE_LIST_ENTRY( failure_entry,failure_action )				\
		{(failure_entry),(failure_action)},

#define	FAILURE_LIST_END( )												\
			{FAIL_NONE,FAIL_ACTION_NONE}};


#define	NONLINEARITY_LIST_BEGIN( nonlin_rec_name )						\
		NONLINEARITY	nonlin_rec_name[]={

#define	NONLINEARITY_LIST_ENTRY( x_coord_tic,	y_coord_tic, tic_value)	\
		{{(x_coord_tic),(y_coord_tic)},(tic_value),0},

#define	NONLINEARITY_LIST_END( )										\
			};
...
#endif
/*// This is the module's export table definition
typedef PGAUGEHDR GAUGESLINKAGE[], *PGAUGESLINKAGE[], **PPGAUGESLINKAGE[];
EXTERN_C DECLSPEC_EXPORT	GAUGESLINKAGE	Linkage;

		EXTERN_C void FSAPI	module_init(void);							
		EXTERN_C void FSAPI	module_deinit(void);						
		EXTERN_C BOOL WINAPI DllMain (HINSTANCE hDLL, DWORD dwReason, LPVOID lpReserved);	
		EXTERN_C PELEMENT_HEADER FSAPI	get_listelement_pointer(PELEMENT_HEADER	pelement,UINT32	pos_element);					
		EXTERN_C void FSAPI	add_imagedata_to_listelement(PELEMENT_HEADER	pelement,UINT32	pos_element,FLAGS image_flags);		
		EXTERN_C void FSAPI	remove_imagedata_from_listelement(PELEMENT_HEADER	pelement,UINT32	pos_element,FLAGS image_flags);	
*/
