...
void Weather::requestWeather(const QString airport, const QString& directory)
{
    ...
    QString url = m_weather_config->getValue(CFG_WEATHER_SERVER);
    url.append(directory);

//    QUrl url2("ftp://pi:raspberry@192.168.1.130/home/pi/proyecto/prueba.txt");
    QUrl url2(url);
    QNetworkRequest req(m_airport+m_weather_config->getValue(CFG_FILE_EXTENSION));
